...
for command in curl sudo tar unzip; do
...
if [[ -z ${DISTRO_NAME:-} || -z ${DISTRO_ID:-} ]]; then
    [[ -r /etc/os-release ]] || {
    ...
    . /etc/os-release
    DISTRO_NAME=${DISTRO_NAME:-${NAME:-${ID:-unknown}}}
    DISTRO_ID=${DISTRO_ID:-${ID:-unknown}}
fi
...
mkdir -p "$HOME/.local/share/fonts/Julia-Mono"
tar -xzf JuliaMono.tar.gz -C "$HOME/.local/share/fonts/Julia-Mono"
curl -o Hasklig.tar.xz -sSL https://github.com/ryanoasis/nerd-fonts/releases/download/v3.4.0/Hasklig.tar.xz
mkdir -p "$HOME/.local/share/fonts/Hasklig-Nerd"
tar -Jxf Hasklig.tar.xz -C "$HOME/.local/share/fonts/Hasklig-Nerd"
...
unzip SourceCodeVar.zip -d "$HOME/.local/share/fonts/Source-Code-Variable"
...
function install_grub {
    ...
    if [[ -f $source_dir/theme.txt ]]; then
        ...
        source_dir=$source_dir/$GRUB_NAME
    elif [[ -n $GRUB_SUBDIR && -f $source_dir/$GRUB_SUBDIR/theme.txt ]]; then
        source_dir=$source_dir/$GRUB_SUBDIR
    elif [[ -n $GRUB_SUBDIR && -f $source_dir/$GRUB_NAME/themes/$GRUB_SUBDIR.tar ]]; then
        mkdir -p "$source_dir/$GRUB_SUBDIR"
        tar -xf "$source_dir/$GRUB_NAME/themes/$GRUB_SUBDIR.tar" -C "$source_dir/$GRUB_SUBDIR"
        source_dir=$source_dir/$GRUB_SUBDIR
    else
        echo "No theme.txt found for $GRUB_NAME" >&2
        return 1
    fi
...
function install_icon {
    ...
    local ICON_REPO=$3
    local ICON_TAG=$4
    local ICON_INSTALL=${5:-false}
    local ICON_ALLDIR=${7:-false}
    local ICON_TEMP_DIR=$BEAUTIFY_DIR/icon/$ICON_NAME
    local -a ICON_INSTALL_ARGS=()
    local -a ICON_PREFIXES=()
    IFS=';' read -r -a ICON_INSTALL_ARGS <<< "${6:-}"
    IFS=';' read -r -a ICON_PREFIXES <<< "${8:-}"
...
function install_theme {
    ...
    local theme_components=(aurorae color-schemes konsole konversation kvantum latte-layout plasma sddm wallpapers yakuake)
    for component in "${theme_components[@]}"; do
        ...
        case $component in
            yakuake)
                source_dir=$source_dir/skins
                target_dir=$target_dir/skins
                ;;
            aurorae|konversation)
                source_dir=$source_dir/themes
                target_dir=$target_dir/themes
                ;;
            sddm)
                source_dir=$source_dir/themes
            ...
            color-schemes)
                [[ -e $source_dir ]] ||
                    source_dir=$(dirname "$source_dir")/colorschemes
                ;;
            latte-layout)
                target_dir=$XDG_CONFIG_HOME/latte
                ;;
            wallpapers)
                [[ -e $source_dir ]] ||
                    source_dir=$(dirname "$source_dir")/wallpaper
                ;;
            kvantum)
                [[ -e $source_dir ]] ||
                    source_dir=$(dirname "$source_dir")/Kvantum
                target_dir=$XDG_CONFIG_HOME/Kvantum
                ;;
        esac
        ...
        case "$target_dir" in
            "$HOME"/*) sudo chown -R "$(id -u):$(id -g)" "$target_dir" ;;
        esac
    done
}
...
function install_plugin {
    ...
    local PLUGIN_REPO=$4
    local PLUGIN_TAG=$5
    local PLUGIN_CATEGRAY=${6:-Applet}
